...
def load_and_preprocess_data(feature, labels, batch_size, window_length, stride, sub_seq_length, num_sub_seqs):
    # Combine A and B
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for batch_idx, (data, bold, target) in enumerate(train_loader):
        data, bold, target = data.to(device), bold.to(device), target.squeeze(1).to(device)
        optimizer.zero_grad()
...
def main():
    ...
    # A = np.random.rand(100, 197, 90)
    # B = np.random.rand(100, 197, 90)
    ...
    # Hyperparameters
    input_dim = feature.shape[2]  # Number of brain regions
    hidden_dim = 64
    ...
    train_loader, test_loader = load_and_preprocess_data(feature, labels, batch_size, window_length, stride, sub_seq_length,
                                                         num_sub_seqs)
